...
export const useAdaStore = create((set, get) => ({
  ...
  loadStoredVoiceName: () => {
    if (typeof window !== 'undefined') {
      try {
        const stored =
          localStorage.getItem('ultron_voice_name') ||
          localStorage.getItem('jarvis_voice_name') ||
          localStorage.getItem('ada_voice_name');
        if (stored) {
          set((state) => ({
            operatorProfile: {
