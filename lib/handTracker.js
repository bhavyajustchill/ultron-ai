...
export class HandTracker {
  ...
  processHands(landmarks, labels) {
    ...
    landmarks.forEach((lm, i) => {
      ...
      // Mirrored so hand-right = screen-right from user's perspective
      const raw = {
    ...
    // Reset reference points on mode change to avoid sudden jumps
  ...
  drawOverlay(landmarks) {
    ...
    for (const lm of landmarks) {
      ...
      ctx.strokeStyle = pinched ? "#ffcc66" : "rgba(255,184,0,0.5)";
      ...
      ctx.fillStyle = pinched ? "#ffcc66" : "rgba(255,184,0,0.7)";
      for (const [x, y] of [
        [tx, ty],
        [ix, iy],
      ]) {
